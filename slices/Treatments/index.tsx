				...
				<div className="flex flex-col gap-10 md:flex-row md:gap-5">
					{items.map((item, index) => (
						<a
							key={index}
							href={
								(item.treatment as unknown as { data: { available: string } }).data.available
									? `/${(item.treatment as unknown as { uid: string }).uid}`
									: undefined
							}
							className="h-426 relative flex-1">
							<img
								src={
									(item.treatment as unknown as { data: { image: { url: string } } }).data?.image
										.url
								}
								alt="Image 1"
							...
							<div className="absolute inset-x-3 bottom-2.5 flex items-center justify-between rounded-xl bg-black px-4 py-2">
								<p className="text-2xl font-semibold text-white">
									{(item.treatment as unknown as { data: { title: string } }).data?.title}
								</p>
